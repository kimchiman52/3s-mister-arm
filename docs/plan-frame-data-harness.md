...
| Headless run | SDL `dummy` video driver is already the **default** (`config.c:34-35, 73`; `sdl_app.c:1817-1818`); `SDL_VIDEODRIVER` env honored (`sdl_app.c:1596`). NOTE: `--headless` CLI flag is a parsed no-op (`OPT_BOOLEAN(0, "headless", ...)` at `args.c:298`, zero consumers) — don't use it, don't trust it | Working via env/default |
