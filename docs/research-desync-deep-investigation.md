...
| `bcdext` (s16) | `sf33rd/Source/Game/effect/effa5.c:15` | set to 0 at `effa5.c:71` before `sbcd()` call each tick | read in `sbcd()` at `effa5.c:20,30,32` | LOW — reset before each `sbcd` call; never read when `effect_A5_move` is not active (battle has no A5 effect). |
