...
- Active queue: keep Loop 184's full-window Yun-only `frame-skip` extension as the new burst-fidelity baseline, then stay on the same user-approved render-only path with a broader trusted burst-only cohort or super-specific thinning follow-up while leaving Ken/Chun matrix harness repair as a separate setup queue
...
- Trusted Yun is still not close enough to stable `60 FPS` to stop at this keep. The next runtime pass should stay on the same burst-fidelity family and test a broader trusted burst-only cohort or super-specific thinning atop the kept full-window `frame-skip` baseline, while keeping Ken/Chun harness repair separate.
...
- Broader trusted burst-only cohort or super-specific thinning follow-up on top of the kept full-window `frame-skip` baseline
- Why it is still live: Loop 184 already covers the full trusted `124`-frame Yun active window and materially improves the formerly uncovered tail, but the deciding lane still sits at `52.2134 FPS` across the full active span instead of stable `60`. The next runtime lever should stay on the same user-approved render-only family and cut more burst work, not reopen another window-length tweak.
...
- Broader trusted burst-only cohort or super-specific thinning on top of the kept full-window `frame-skip` baseline
- Lever class: `workload-fidelity`
- Expected upside: Loop 184 already converted the full trusted active window into a stronger Yun keep, so the next remaining headroom is still on the same user-approved burst-fidelity path rather than in window length. A broader cohort or super-specific thinning could reduce the rendered half of the skipped window further and move trusted Yun closer to stable speed
- Main risk: broadening the cohort or adding super-specific rules can leak beyond the intended burst-visual scope, so the next loop still needs the same direct-route checks plus Q/idle guards
...
- One scoped change: stay on the same render-only super-fidelity path and test one bounded stronger follow-up on `yun-sa3-repeat-pressure` beyond the now-closed window-length lever, either by broadening the trusted burst-only cohort on rendered frames or by adding super-specific thinning on top of the kept full-window reuse; keep Q SA1 and `gameplay-idle` as guards; do not repair the Ken/Chun harness in the same loop
...
- Capture plan: keep `yun-sa3-repeat-pressure` as the deciding lane, start the repeat-super captures at `game-input-active` with zero warmup so the trigger lands inside the sample, and continue judging first `8`, first `60`, first `82`, the post-`82` active tail, and the full active window; rerun Q SA1 as the secondary live guard; do not trust Ken/Chun matrix outcomes again until the harness proves `p1_super_active_starts > 0`
- Keep if: the broader or super-specific follow-up materially improves the full trusted active window beyond kept Loop 184 while preserving the same direct/native route and without broad Q/idle regression
