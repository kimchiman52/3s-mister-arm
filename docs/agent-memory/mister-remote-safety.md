...
- The OSD launcher step of `deploy` writes into `/media/fat/Scripts/`, which is shared with every other core. It installs exactly `3S-ARM.sh` and deletes only launcher names a previous deploy recorded in `<runtime root>/.osd-scripts-manifest`. Nothing else in that directory is ever a delete candidate. See `mister_deploy_osd_launcher` in `tools/mister/mister-common.sh`.
...
- Decision: the same delete-only-what-you-own rule applies to `/media/fat/Scripts/`, via its own `.osd-scripts-manifest`. | Why: the manifest fix for the runtime root did not reach it — `/media/fat/Scripts` sits outside the runtime root, so the OSD launcher step kept a hardcoded `rm -f` naming `3S-ARM_Training_Yun_Ryu_Ryu_Stage.sh` and `3S-ARM Training Yun Ryu Ryu Stage.sh`. Nothing in this repo has ever created either; they were hand-authored training shortcuts, destroyed on every deploy, and the runbook documented that as expected behaviour. Third instance of the same premise. Narrowing the list to a `3S-ARM_Training_*` glob would have kept it. | Date: `2026-08-29`.
