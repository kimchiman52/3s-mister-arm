...
- When editing `/Users/sb/Developer/3sx-mister/tools/mister/*`.
- When editing `/Users/sb/Developer/3sx-mister/tools/mister-wrapper/*` packaging or deploy flows.
...
- Entry points: `/Users/sb/Developer/3sx-mister/tools/mister/mister-common.sh`, `/Users/sb/Developer/3sx-mister/tools/mister/misterctl.sh`, `/Users/sb/Developer/3sx-mister/tools/mister/perf-sampler.sh`.
...
- Safe remote delete scope: `/media/fat/games/3sx/` only.
- Safe wrapper copy-only targets: `/media/fat/MiSTer_3SX` and `/media/fat/_Other/3SX.rbf`.
...
- Collision valve: `MISTER_ALLOW_BUSY_TARGET=1` only when you deliberately accept running against a target that already shows active `3sx`, `MiSTer_3SX`, transfer, or launcher processes.
...
- Decision: wrapper deploys may copy into `/media/fat`, but delete is only allowed inside `/media/fat/games/3sx/`. | Why: wrapper assets own only three targets and should not mutate the rest of the card. | Date: `2026-03-10`.
- Decision: nonstandard remote roots require an explicit unsafe override. | Why: path typos like `/media/fat` vs `/media/fat/games/3sx` are high-impact. | Date: `2026-03-10`.
...
- `rsync --delete` aimed at `/media/fat/` or another broad root -> reject it or narrow the destination to `/media/fat/games/3sx/`.
