...
3. A byte-swap / wrap-around in `Battle_Country`'s u8 vs `bg_w.stage`'s type. Grep: `struct BG.stage` is `u8` (see `src/sf33rd/Source/Game/stage/bg.h`). `Battle_Country` is `u8` in game_state.h:385 — same.
