...
|---|---|---|
| `FAILED_STUN` | "Connection failed. Try again." | host: direct_p2p.c:266, 1025; thread-spawn failure paths in Begin* :1870/:1930; joiner: :1096 |
| `FAILED_PUNCH` | "Invalid room code." | BeginJoin decode failures, direct_p2p.c:3814, 4398 |
...
(`try_portmap`, direct_p2p.c:1986-2062) and its external IP is not
...
| **Full/restricted cone, no UPnP** | direct punch succeeds | succeeds | joiner's source port differs per destination → host's echo goes to the STUN-observed (wrong) port; **bilateral fallback required**, host-side learns true port from first inbound (host_tick_receive captures source, direct_p2p.c:1762-1781) |
...
(`host_stun_keepalive_tick`, direct_p2p.c:3389-3405;
...
direct_p2p.c:3458-3498), and restart the rendezvous loop under the new
...
`upnp_renew_tick`, direct_p2p.c:2673-2820), retry at 5 min on failure,
...
- **Auto-retry policy**: (a) joiner — join_thread_fn
  ...
  Tick's FAILED_STUN case (direct_p2p.c:3226) re-spawns
...
- `classify_host_datagram` (direct_p2p.c:119) is the single routing
  ...
  (host_tick_receive, direct_p2p.c:2356-2378): the peer slot is never
...
echoes immediately (`host_handle_challenge`, direct_p2p.c:3781).
...
- **Sizing is a network quantity.** With start skew `s`, one-way delay
  ...
  (`9240aa50:src/netplay/direct_p2p.c:1153-1176`); only the trigger
...
- The bound that does apply is nav's `NAV_WAIT_ORCH_TIMEOUT_FRAMES`
  ...
  `netplay_nav.c:391`). 18 400 ms of race against 150 000 ms is **8.2x
  headroom**, and the +2 400 ms the two tail exemptions add across two
  attempts is 1.6% of it. The *failure* path — the one a user actually
  waits through, since a success hands off — is unchanged at 17 200 ms.
...
- **How often a real pair lands in the band was NOT measured — only
  ...
  - The host enters the race **only** from `try_handle_deliver`, and
    ...
    (`src/netplay/direct_p2p.c:3654`, gate at `:3658`, spawn at
    `:3751`).
  ...
  - When it is **lost**, the host learns only from the reply DELIVER to
    ...
    `src/netplay/direct_p2p.c:2521`). One lost datagram therefore places
...
- **Teardown dispatches** through `portmap_remove` (direct_p2p.c:2205),
...
- **The renewal interval now follows the granted lease**
  (`portmap_renew_interval_ms`, direct_p2p.c:2330). RFC 6886 §3.3: "The
...
- **(L-3) The PCP nonce globals are single-writer by convention, and a
  ...
  `SDL_DetachThread` (`direct_p2p.c:2230`) abandons the timed-out
  worker, and `try_portmap` returns false **without** adopting the
  result — the `s_upnp_mapping` assignment is on the joined path only
  (`direct_p2p.c:2695`) — so `s_upnp_mapping.active` stays false; the
  FAILED_STUN auto-retry re-spawns `host_thread_fn`
  (`direct_p2p.c:4495`); the "reuse the live mapping" shortcut is gated
  on that same `s_upnp_mapping` flag (`direct_p2p.c:2805`) and is
  therefore skipped; and `try_portmap` runs again (`direct_p2p.c:1968`),
