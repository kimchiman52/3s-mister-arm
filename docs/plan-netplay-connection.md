...
|---|---|---|
| `DIRECT_P2P_FAILED_STUN` | "Connection failed. Try again." | host: direct_p2p.c:3152, 1025; thread-spawn failure paths in Begin* :1870/:1930; joiner: :1096 |
| `DIRECT_P2P_FAILED_PUNCH` | "Invalid room code." | BeginJoin decode failures, direct_p2p.c:4579, 4398 |
...
(`try_portmap`, direct_p2p.c:2321-2396) and its external IP is not
...
| **Full/restricted cone, no UPnP** | direct punch succeeds | succeeds | joiner's source port differs per destination → host's echo goes to the STUN-observed (wrong) port; **bilateral fallback required**, host-side learns true port from first inbound (host_tick_receive captures source, direct_p2p.c:1836-1855) |
...
`host_rendezvous_thread_fn` (direct_p2p.c:875-966) now re-REGISTERs
...
(`host_stun_keepalive_tick`, direct_p2p.c:3846-3862;
...
`host_tick_receive` (direct_p2p.c:1825-1835) — which also fixes a
...
direct_p2p.c:3915-3955), and restart the rendezvous loop under the new
...
`upnp_renew_tick`, direct_p2p.c:2587-2734), retry at 5 min on failure,
...
discovery (direct_p2p.c:1049-1084): the mapping is dropped only when
...
- **Auto-retry policy**: (a) joiner — join_thread_fn
  ...
  Tick's FAILED_STUN case (direct_p2p.c:5037) re-spawns
...
- `classify_host_datagram` (direct_p2p.c:126) is the single routing
  ...
  (host_tick_receive, direct_p2p.c:2430-2452): the peer slot is never
...
8-byte cookie crosses via a seqlock (`signal_cookie_publish`, direct_p2p.c:668;
`signal_cookie_snapshot`, direct_p2p.c:683) and the main thread also
echoes immediately (`host_handle_challenge`, direct_p2p.c:4226).
...
- **Sizing is a network quantity.** With start skew `s`, one-way delay
  ...
  (`9240aa50:src/netplay/direct_p2p.c:1227-1250`); only the trigger
...
- The bound that does apply is nav's NAV_WAIT_ORCHESTRATOR deadline
  ...
  `DirectP2P_OrchWorstCaseMsForRole` (`direct_p2p.c:5420`). At the
...
- **How often a real pair lands in the band was NOT measured — only
  ...
  - The host enters the race **only** from `try_handle_deliver`, and
    ...
    (`src/netplay/direct_p2p.c:3750`, gate at `:3658`, spawn at
    `:3751`).
  ...
  - When it is **lost**, the host learns only from the reply DELIVER to
    ...
    `src/netplay/direct_p2p.c:2595`). One lost datagram therefore places
...
- **(L-4) The H-3 relay grace is suppressed in exactly the case it was
  ...
  `budget_done` computed at `direct_p2p.c:2096` and the deferral to
  `relay_grace_pending` at `direct_p2p.c:2168` — the two bounds are
...
- **Teardown dispatches** through `portmap_remove` (direct_p2p.c:2255),
...
- **The renewal interval now follows the granted lease**
  (`portmap_renew_interval_ms`, direct_p2p.c:2472). RFC 6886 §3.3: "The
...
- **(L-3) The PCP nonce globals are single-writer by convention, and a
  ...
  `SDL_DetachThread` (`direct_p2p.c:2382`) abandons the timed-out
  worker, and `try_portmap` returns false **without** adopting the
  result — the `s_upnp_mapping` assignment is on the joined path only
  (`direct_p2p.c:2392`) — so `s_upnp_mapping.active` stays false; the
  FAILED_STUN auto-retry re-spawns `host_thread_fn`
  (`direct_p2p.c:5089`); the "reuse the live mapping" shortcut is gated
  on that same `s_upnp_mapping` flag (`direct_p2p.c:3063`) and is
  therefore skipped; and `try_portmap` runs again (`direct_p2p.c:3080`),
