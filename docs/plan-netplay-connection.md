...
|---|---|---|
| `DIRECT_P2P_FAILED_STUN` | "Connection failed. Try again." | host: direct_p2p.c:3564, 1146; thread-spawn failure paths in Begin* :1991/:2051; joiner: :1217 |
| `DIRECT_P2P_FAILED_PUNCH` | "Invalid room code." | BeginJoin decode failures, direct_p2p.c:5166, 4574 |
...
(`try_portmap`, direct_p2p.c:2446-2521) and its external IP is not
...
| **Full/restricted cone, no UPnP** | direct punch succeeds | succeeds | joiner's source port differs per destination → host's echo goes to the STUN-observed (wrong) port; **bilateral fallback required**, host-side learns true port from first inbound (host_tick_receive captures source, direct_p2p.c:1945-1964) |
...
`host_rendezvous_thread_fn` (direct_p2p.c:989-1080) now re-REGISTERs
...
(`host_stun_keepalive_tick`, direct_p2p.c:4415-4431;
...
`host_tick_receive` (direct_p2p.c:1934-1944) — which also fixes a
...
direct_p2p.c:4484-4524), and restart the rendezvous loop under the new
...
`upnp_renew_tick`, direct_p2p.c:2897-3050), retry at 5 min on failure,
...
discovery (direct_p2p.c:1163-1198): the mapping is dropped only when
...
- **Auto-retry policy**: (a) joiner — join_thread_fn
  ...
  Tick's FAILED_STUN case (direct_p2p.c:5581) re-spawns
...
- `classify_host_datagram` (direct_p2p.c:126) is the single routing
  ...
  (host_tick_receive, direct_p2p.c:2539-2561): the peer slot is never
...
8-byte cookie crosses via a seqlock (`signal_cookie_publish`, direct_p2p.c:784;
`signal_cookie_snapshot`, direct_p2p.c:799) and the main thread also
echoes immediately (`host_handle_challenge`, direct_p2p.c:4795).
...
- The bound that does apply is nav's NAV_WAIT_ORCHESTRATOR deadline
  ...
  `DirectP2P_OrchWorstCaseMsForRole` (`direct_p2p.c:6214`). At the
...
- **How often a real pair lands in the band was NOT measured — only
  ...
  - The host enters the race **only** from `try_handle_deliver`, and
    ...
    (`try_handle_deliver` at `src/netplay/direct_p2p.c:4680`,
  ...
  - When it is **lost**, the host learns only from the reply DELIVER to
    ...
    `src/netplay/direct_p2p.c:2889`). One lost datagram therefore places
...
- **Teardown dispatches** through `portmap_remove` (direct_p2p.c:2447),
...
- **The renewal interval now follows the granted lease**
  (`portmap_renew_interval_ms`, direct_p2p.c:2849). RFC 6886 §3.3: "The
...
- **(L-3) The PCP nonce globals are single-writer by convention, and a
  ...
  `SDL_DetachThread` (`direct_p2p.c:2574`) abandons the timed-out
  ...
  therefore skipped; and `try_portmap` runs again (`direct_p2p.c:3482`),
