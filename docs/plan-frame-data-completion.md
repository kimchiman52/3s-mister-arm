...
| **ARCADE-VS-PORT-DIVERGENCE** | 2 | q-crmp-hit-capture-a, q-crmp-hit-capture-b (both in `corpus-q-arcade.yaml`, added 2026-08-30 by task #108). NOT terminal — an OPEN work item. Under arcade balance both measure `adv=-2` where the PS2 twin measures the oracle's `-1`, outcome/S/A/R identical. The class is new because the engine it reports on is new to the suite: arcade balance was pinned off for every test-runner process until #108. Tracked in `docs/queue.md`; reopen condition in the corpus note |
| **Total** | **55**<!-- canon:xfail --> | |

**(Census reconciliation, dated 2026-08-30 — task #108.)** The rows above now sum to **55** (15+12+10+4+6+4+2+**2**), matching the **Total** row and the current suite count (1,348 PASS / 55 XFAIL over 96 corpora). The +2 is the new ARCADE-VS-PORT-DIVERGENCE class: none of the 53 below moved, and nothing regressed — two rows of NEWLY REACHABLE arcade-engine coverage report a disagreement that no corpus could previously have seen. Mirrors the twin-table reconciliation in `docs/frame-data-synthesis.md` §13.16.
