...
5. **argparse `ARGPARSE_OPT_BOOLEAN` 4-byte RMW through `bool*`.**
   ...
   `OPT_BOOLEAN` sites, e.g. `args.c:262` `&configuration->headless`, `args.c:430`
...
(23 sites; e.g. `&configuration->headless` at `args.c:262`,
`&configuration->test.enabled` at `args.c:430`,
`&configuration->test.initial_super_full` at `args.c:483`). Reading 3 bytes of
...
The **Total** row below is a live figure carrying an invisible `canon:xfail` marker; it is checked against the summed golden tables by `python3 tools/frame-data/check-canon-numbers.py --check`. See `docs/frame-data-synthesis.md` § "Verification canon (live)" for the full canon and the rule that dated log figures elsewhere in this file are historical records and must not be edited to match the present.
...
| **ARCADE-VS-PORT-DIVERGENCE** | 0 | q-crmp-hit-capture-a, q-crmp-hit-capture-b (both in `corpus-q-arcade.yaml`, opened 2026-08-30 by task #108, CLOSED the same day by task #136). Cause isolated: arcade skips `check_omop_vital` (`plmain.c:335-337`), so the training dummy is never healed and reaches zero vitality partway through the corpus, at which point `same_dm_stop`'s nearly-dead branch (`hitcheck.c:1023-1039`) shortens the defender hitstop by one frame. `att.dipsw`/`hs_me`/`hs_you` are identical on both engines — not a balance difference. Fixed in the harness (`input_script_restore_vitality`, `src/test/input_script.c`); both rows now PASS at the oracle's `adv=-1`. Row kept at 0 so the class and its reopen condition stay on the record |
| **Total** | **53**<!-- canon:xfail --> | |

**(Census reconciliation, dated 2026-08-30 — task #136.)** The rows above now sum to **53** (15+12+10+4+6+4+2+**0**), matching the **Total** row and the current suite count (1,435 PASS / 53 XFAIL over 99 corpora). ARCADE-VS-PORT-DIVERGENCE opened at 2 earlier the same day and closed at 0 once its cause was isolated (the arcade training dummy reaching zero vitality — `check_omop_vital` is arcade-skipped at `plmain.c:335-337`, `same_dm_stop`'s nearly-dead branch at `hitcheck.c:1023-1039` then shortens the defender hitstop by a frame); both rows now PASS at the oracle. None of the 53 moved in either direction across #108 or #136. Mirrors the twin-table reconciliation in `docs/frame-data-synthesis.md` §13.16.

**(Census reconciliation, dated 2026-08-30 — task #108; HISTORICAL, superseded by the #136 note above the same day; left as written.)** The rows above then summed to **55** (15+12+10+4+6+4+2+**2**), matching the **Total** row and the then-current suite count (1,348 PASS / 55 XFAIL over 96 corpora). The +2 was the new ARCADE-VS-PORT-DIVERGENCE class: none of the 53 below moved, and nothing regressed — two rows of NEWLY REACHABLE arcade-engine coverage reported a disagreement that no corpus could previously have seen.

**(Census reconciliation, dated 2026-07-17 — HISTORICAL, superseded by the 2026-08-30 note above; left as written.)** The rows above then summed to 53 (15+12+10+4+6+4+2), matching the **Total** row and the post-lever-W suite count (1,296 PASS / 53 XFAIL); the earlier figure of 55 is superseded by the lever-W yang-senkyuutai-lk-block + yang-exsenkyuutai-block XFAIL→PASS flip (TERMINAL-NON-SURGICAL 17→15), which itself superseded 59 via the lever-V remy-sa1 ×4 flip (CAPABILITY-GATED-remaining 8→4). Mirrors the twin-table reconciliation in `docs/frame-data-synthesis.md` §13.16.
