...
| P-2.3 `direct_p2p.c` needs `NETPLAY_TEST_HOOKS` seam in Step 6 | **Addressed** | §Step 6 "Changes" now explicitly lists `src/netplay/direct_p2p.c` as a modified file, cites the `Stun_HolePunch` call site (`direct_p2p.c` line 421 as the file then stood), and documents the function-pointer indirection pattern. *[The seam landed and then moved: S6 replaced that override with a punch-oracle seam — `DirectP2P_TestHook_SetPunchOracle` / `PUNCH_ORACLE`, `src/netplay/direct_p2p.c:1089-1095` — because the punch no longer owns the socket for a whole window.]* |
