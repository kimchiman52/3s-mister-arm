...
**Why the revision:**
- 32-bit vs 64-bit crossplay is definitively broken (see research doc §9.7 — cross-architecture compatibility section, added 2026-04-20). `SessionHealthMsg.checksum` hashes `sizeof(PLW)` raw bytes, which differs between ILP32 and LP64 due to embedded pointer fields → cross-arch matches die within seconds via `GekkoDesyncDetected`.
