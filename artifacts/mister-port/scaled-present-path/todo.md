...
- [x] Combined experiment order for Ralph:
  ...
  - if the chosen loop is broader than nearest, prefer gameplay-wide raster residue before reopening menu or title work, and treat `basic-exchange --test-stage 7` as the first ordinary no-super raster gate ahead of presenter-only `basic-exchange` or `pressure-exchange`
