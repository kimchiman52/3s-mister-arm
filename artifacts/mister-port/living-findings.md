...
- Active checklist: `artifacts/mister-port/scaled-present-path/todo.md` remains the branch-default modern-display nearest checklist on `nearest-hdmi-perf`, but the `2026-03-18` native regular-gameplay override explicitly reactivates `artifacts/mister-port/stock-image-software-frame-loop-series/todo.md` as the canonical checklist for this automation run; keep `artifacts/mister-port/ralphpart2/todo.md` as historical context only
...
- 2026-03-21 native next-step order for Ralph loops: `(1)` audit and, if justified, admit the small `ppg-seqs ix 80 / texture 56` generic residue with a narrow micro-lookup path under the shared `384`-pixel gate; `(2)` then test one careful scalar `4x` unroll in the non-integer row-walk gather loop while preserving the kept pair-only reuse path; `(3)` keep Remy-left on separate compare-dirty `ppg-seqs 81/82` residue work. Do not spend the next native override loops on presenter-side native work, dual-core raster threading, broader run batching beyond pairs, or runtime row-dedup before measurement.
