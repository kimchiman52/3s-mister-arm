...
- Active nearest hotspot matrix: keep `control`, `stage-heavy`, `ibuki-stage7`, `2p-character-select`, and `menu-transition` in the nearest-HDMI validation pool instead of relying only on the original two gameplay gates. `genei-jin-first-activation` remains a required hotspot too, but only as a manual or future measurement-support lane until a trustworthy unattended capture exists on this branch.
