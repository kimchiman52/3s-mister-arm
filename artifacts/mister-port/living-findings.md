...
- Current nearest HDMI direct-path baseline: the kept `r18` runtime still reproduces cleanly on the live `1920x1080` device as `r19`, with gameplay gates staying on `software_frame_mapped_scale` rather than route fallback (`nearest-hdmi-r19-control-full = 71.2908 FPS / 14.0271 ms / 3.0005 ms present_copy`, `nearest-hdmi-r19-stage-heavy-full = 52.2327 FPS / 19.1451 ms / 4.0327 ms present_copy`, `nearest-hdmi-r19-ibuki-stage7-basic = 57.2269 FPS / 17.4743 ms / 6.8739 ms present`, `native-hdmi-r19-control-basic = 92.7239 FPS / 10.7847 ms / 0.5450 ms present`). The remaining user-facing nearest slowdown is therefore current direct-path copy/raster cost, not a regression back to `fullscreen_staging` or readback.
