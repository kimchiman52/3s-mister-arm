...
- On `nearest-hdmi-perf`, the nearest scaled-present checklist stays active for modern HDMI follow-up. The kept dirty-row mapped-present reland removes most of the old full-frame copy tax, but nearest still trails native on `1920x1080`; if another loop is needed, stay inside partial mapped-present follow-up (dirty spans or source-tile-guided copies) rather than route recovery.
