...
- Native override queue for the current Ralph automation run: use `artifacts/mister-port/stock-image-software-frame-loop-series/todo.md` and the trusted native baselines `loop132-yun-family-time-r2` plus `loop132-remy-left-family-time`, not the nearest-HDMI ranking below. The native next-step order is the same March `2026-03-21` split recorded in the current snapshot: `ix 80 / texture 56` generic-residue micro-lookup audit first, scalar `4x` non-integer row-walk unroll second, and Remy-left compare-dirty `81/82` residue as a separate track.
