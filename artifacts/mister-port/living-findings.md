...
- Active nearest hotspot matrix: keep `control`, `stage-heavy`, and `ibuki-stage7` as the gameplay-first nearest-HDMI gates. `2p-character-select` and `menu-transition` are still useful guardrails, but they should not outrank gameplay FPS for loop budget or keep/reject decisions. `genei-jin-first-activation` is the top missing automated gameplay lane on this branch.
- Genei-Jin automation status: the old Yun helper plumbing still exists on this branch (`--test-p1-super-full`, `--test-delay-gameplay-inputs-until-active`, `--perf-wait-test-phase game-input-active`), but scripted Yun captures are not trustworthy coverage until they prove nonzero `p1_super_art_active_frames_total` and catch the first activation inside the sampled window rather than only a stocked pre-activation setup or a later easier burst.
