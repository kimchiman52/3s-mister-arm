...
static void compare_main_values(SDL_IOStream* io) {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        const Position pos_3sarm = get_position(i);
        const Position pos_cps3 = read_position(io, i);
        stop_if(pos_3sarm.x != pos_cps3.x);
        stop_if(pos_3sarm.y != pos_cps3.y);

        // if (i == 0) {
        //     printf("🔴 %llu pos x: %d vs %d\n", frame, pos_cps3.x, pos_3sarm.x);
        // }

        const s16 vital_new_3sarm = plw[i].wu.vital_new;
        const s16 vital_new_cps3 = read_s16(io, plw_offset + WORK_VITAL_NEW_OFFSET);
        stop_if(vital_new_3sarm != vital_new_cps3);

        const s16 stun_3sarm = piyori_type[i].now.quantity.h;
        const s16 stun_cps3 = read_s16(io, PIYORI_TYPE_OFFSET + i * sizeof(PiyoriType) + offsetof(PiyoriType, now));
        stop_if(stun_3sarm != stun_cps3);

        const s16 sa_gauge_3sarm = super_arts[i].gauge.s.h;
        const s16 sa_gauge_cps3 = read_s16(io, SUPER_ARTS_WORK_OFFSET + i * sizeof(SA_WORK) + offsetof(SA_WORK, gauge));
        stop_if(sa_gauge_3sarm != sa_gauge_cps3);

        const s16 sa_store_3sarm = super_arts[i].store;
        const s16 sa_store_cps3 = read_s16(io, SUPER_ARTS_WORK_OFFSET + i * sizeof(SA_WORK) + offsetof(SA_WORK, store));
        stop_if(sa_store_3sarm != sa_store_cps3);
    }
}
...
static void compare_service_values(SDL_IOStream* io, bool compare_characters, Uint64 frame) {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        const u8 caution_flag_3sarm = plw[i].caution_flag;
        const u8 caution_flag_cps3 = read_u8(io, plw_offset + PLW_CAUTION_FLAG_OFFSET);
        stop_if(caution_flag_3sarm != caution_flag_cps3);

        const u8 do_not_move_3sarm = plw[i].do_not_move;
        const u8 do_not_move_cps3 = read_u8(io, plw_offset + PLW_DO_NOT_MOVE_OFFSET);
        stop_if(do_not_move_3sarm != do_not_move_cps3);

        for (int j = 0; j < 8; j++) {
            const s16 routine_no_3sarm = plw[i].wu.routine_no[j];
            const s16 routine_no_cps3 = read_s16(io, plw_offset + WORK_ROUTINE_NO_OFFSET + j * 2);
            stop_if(routine_no_3sarm != routine_no_cps3);
        }

        const s16 dm_stop_3sarm = plw[i].wu.dm_stop;
        const s16 dm_stop_cps3 = read_s16(io, plw_offset + WORK_DM_STOP_OFFSET);
        stop_if(dm_stop_3sarm != dm_stop_cps3);

        const s16 hit_stop_3sarm = plw[i].wu.hit_stop;
        const s16 hit_stop_cps3 = read_s16(io, plw_offset + WORK_HIT_STOP_OFFSET);
        stop_if(hit_stop_3sarm != hit_stop_cps3);

        const u8 sa_stop_flag_3sarm = plw[i].sa_stop_flag;
        const u8 sa_stop_flag_cps3 = read_u8(io, plw_offset + PLW_SA_STOP_FLAG_OFFSET);
        stop_if(sa_stop_flag_3sarm != sa_stop_flag_cps3);

        // const u16 cg_ix_cps3 = read_u16(io, plw_offset + WORK_CG_IX_OFFSET);
        // const u16 cg_ix_3sarm = plw[i].wu.cg_ix;
        // stop_if(cg_ix_cps3 != cg_ix_3sarm);

        const u16 cg_add_xy_cps3 = read_u16(io, plw_offset + WORK_CG_ADD_XY_OFFSET);
        const u16 cg_add_xy_3sarm = plw[i].wu.cg_add_xy;
        stop_if(cg_add_xy_3sarm != cg_add_xy_cps3);
    }
}
...
static void compare_lvr(SDL_IOStream* io) {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        const T_PL_LVR* lvr_3sarm = &t_pl_lvr[i];

        stop_if(lvr_3sarm->sw_new != lvr_cps3->sw_new);
        stop_if(lvr_3sarm->sw_old != lvr_cps3->sw_old);
        stop_if(lvr_3sarm->sw_chg != lvr_cps3->sw_chg);
        stop_if(lvr_3sarm->sw_now != lvr_cps3->sw_now);
        stop_if(lvr_3sarm->old_now != lvr_cps3->old_now);
        stop_if(lvr_3sarm->now_lvbt != lvr_cps3->now_lvbt);
        stop_if(lvr_3sarm->old_lvbt != lvr_cps3->old_lvbt);
        stop_if(lvr_3sarm->new_lvbt != lvr_cps3->new_lvbt);
        stop_if(lvr_3sarm->sw_lever != lvr_cps3->sw_lever);
        stop_if(lvr_3sarm->shot_up != lvr_cps3->shot_up);
        stop_if(lvr_3sarm->shot_down != lvr_cps3->shot_down);
        stop_if(lvr_3sarm->shot_ud != lvr_cps3->shot_ud);
        stop_if(lvr_3sarm->lvr_status != lvr_cps3->lvr_status);
        stop_if(lvr_3sarm->jaku_cnt != lvr_cps3->jaku_cnt);
        stop_if(lvr_3sarm->chuu_cnt != lvr_cps3->chuu_cnt);
        stop_if(lvr_3sarm->kyou_cnt != lvr_cps3->kyou_cnt);
        stop_if(lvr_3sarm->up_cnt != lvr_cps3->up_cnt);
        stop_if(lvr_3sarm->down_cnt != lvr_cps3->down_cnt);
        stop_if(lvr_3sarm->left_cnt != lvr_cps3->left_cnt);
        stop_if(lvr_3sarm->right_cnt != lvr_cps3->right_cnt);
        stop_if(lvr_3sarm->s1_cnt != lvr_cps3->s1_cnt);
        stop_if(lvr_3sarm->s2_cnt != lvr_cps3->s2_cnt);
        stop_if(lvr_3sarm->s3_cnt != lvr_cps3->s3_cnt);
        stop_if(lvr_3sarm->s4_cnt != lvr_cps3->s4_cnt);
        stop_if(lvr_3sarm->s5_cnt != lvr_cps3->s5_cnt);
        stop_if(lvr_3sarm->s6_cnt != lvr_cps3->s6_cnt);
        stop_if(lvr_3sarm->lu_cnt != lvr_cps3->lu_cnt);
        stop_if(lvr_3sarm->ld_cnt != lvr_cps3->ld_cnt);
        stop_if(lvr_3sarm->ru_cnt != lvr_cps3->ru_cnt);
        stop_if(lvr_3sarm->rd_cnt != lvr_cps3->rd_cnt);
        stop_if(lvr_3sarm->waza_num != lvr_cps3->waza_num);
        // stop_if(lvr_3sarm->waza_no != lvr_cps3->waza_no);
        stop_if(lvr_3sarm->wait_cnt != lvr_cps3->wait_cnt);
        stop_if(lvr_3sarm->cmd_r_no != lvr_cps3->cmd_r_no);
    }
}
...
static void compare_waza_work(SDL_IOStream* io) {
    ...
    for (int i = 0; i < 2; i++) {
        for (int j = 0; j < 56; j++) {
            const WAZA_WORK* w_3sarm = &waza_work[i][j];
            const WAZA_WORK* w_cps3 = &waza_work_cps3[i][j];

            stop_if(w_3sarm->w_type != w_cps3->w_type);
            stop_if(w_3sarm->w_int != w_cps3->w_int);
            stop_if(w_3sarm->free1 != w_cps3->free1);
            stop_if(w_3sarm->w_lvr != w_cps3->w_lvr);
            stop_if(w_3sarm->free2 != w_cps3->free2);
            stop_if(w_3sarm->w_dead != w_cps3->w_dead);
            stop_if(w_3sarm->w_dead2 != w_cps3->w_dead2);
            stop_if(w_3sarm->uni0.tame.flag != w_cps3->uni0.tame.flag);
            stop_if(w_3sarm->uni0.tame.shot_flag != w_cps3->uni0.tame.shot_flag);
            stop_if(w_3sarm->uni0.tame.shot_flag2 != w_cps3->uni0.tame.shot_flag2);
            stop_if(w_3sarm->free3 != w_cps3->free3);
            stop_if(w_3sarm->shot_ok != w_cps3->shot_ok);
        }
    }
}
...
static void compare_wcp(SDL_IOStream* io) {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        const WORK_CP* w_3sarm = &wcp[i];
        const WORK_CP* w_cps3 = &wcp_cps3[i];

        stop_if(w_3sarm->sw_lvbt != w_cps3->sw_lvbt);
        stop_if(w_3sarm->sw_new != w_cps3->sw_new);
        stop_if(w_3sarm->sw_old != w_cps3->sw_old);
        stop_if(w_3sarm->sw_now != w_cps3->sw_now);
        stop_if(w_3sarm->sw_off != w_cps3->sw_off);
        stop_if(w_3sarm->sw_chg != w_cps3->sw_chg);
        stop_if(w_3sarm->old_now != w_cps3->old_now);
        stop_if(w_3sarm->lgp != w_cps3->lgp);
        stop_if(w_3sarm->ca14 != w_cps3->ca14);
        stop_if(w_3sarm->ca25 != w_cps3->ca25);
        stop_if(w_3sarm->ca36 != w_cps3->ca36);
        stop_if(w_3sarm->calf != w_cps3->calf);
        stop_if(w_3sarm->calr != w_cps3->calr);
        stop_if(w_3sarm->lever_dir != w_cps3->lever_dir);

        for (int j = 0; j < 56; j++) {
            stop_if(w_3sarm->waza_flag[j] != w_cps3->waza_flag[j]);

            if (w_3sarm->waza_flag[j] == -1) {
                continue;
            }

            stop_if(w_3sarm->reset[j] != w_cps3->reset[j]);
            stop_if(w_3sarm->btix[j] != w_cps3->btix[j]);

            for (int k = 0; k < 4; k++) {
                stop_if(w_3sarm->waza_r[j][k] != w_cps3->waza_r[j][k]);
                stop_if(w_3sarm->exdt[j][k] != w_cps3->exdt[j][k]);
            }
        }
    }
}
