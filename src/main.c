...
static void read_args(int argc, const char* argv[]) {
    struct argparse_option options[] = {
        ...
        OPT_STRING(0,
                   ...
                   "Optional named scripted gameplay preset (stage-heavy, effect-heavy, super-heavy, yun-sa3-repeat, basic-exchange, pressure-exchange, left-corner-ryu-stage, training-yun-ryu-ryu-stage).",
                   NULL,
                   0,
                   0),
        OPT_INTEGER(0,
...
static void verify_args() {
    ...
    if (!is_supported_test_scene_preset(test->scene_preset)) {
        error_out_with_code(
            "--test-scene-preset must be one of stage-heavy, effect-heavy, super-heavy, yun-sa3-repeat, basic-exchange, pressure-exchange, left-corner-ryu-stage, or training-yun-ryu-ryu-stage.",
            EXIT_CODE_RUNTIME_ERROR);
    }
