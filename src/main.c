...
static void read_args(int argc, const char* argv[]) {
    struct argparse_option options[] = {
        ...
        OPT_STRING(0,
                   ...
                   "Optional named scripted gameplay preset (stage-heavy, effect-heavy, super-heavy, basic-exchange, pressure-exchange, left-corner-ryu-stage, training-yun-ryu-ryu-stage).",
                   NULL,
                   0,
                   0),
        OPT_INTEGER(0,
...
static bool is_supported_test_scene_preset(const char* preset) {
    return preset == NULL || SDL_strcmp(preset, "stage-heavy") == 0 || SDL_strcmp(preset, "effect-heavy") == 0 ||
           ...
           SDL_strcmp(preset, "pressure-exchange") == 0 || SDL_strcmp(preset, "left-corner-ryu-stage") == 0 ||
...
static void verify_args() {
    ...
    if (!is_supported_perf_wait_test_phase(configuration.perf.wait_for_test_phase)) {
        error_out_with_code("--perf-wait-test-phase must be one of title, menu, "
                            ...
                            "game-input-active, p1-super-art-active, or wipe-transition-type1.",
    ...
    if (!is_supported_test_scene_preset(test->scene_preset)) {
        error_out_with_code(
            "--test-scene-preset must be one of stage-heavy, effect-heavy, super-heavy, basic-exchange, pressure-exchange, left-corner-ryu-stage, or training-yun-ryu-ryu-stage.",
            EXIT_CODE_RUNTIME_ERROR);
    }
