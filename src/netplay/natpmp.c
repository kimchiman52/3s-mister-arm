 ...
 *     it is still inside Natpmp_AddMapping (direct_p2p.c:2356), and
 *     try_portmap returns false WITHOUT adopting the result: the
 *     s_upnp_mapping assignment sits on the joined path only
 *     (direct_p2p.c:2450), so s_upnp_mapping.active stays false;
 *   - the FAILED_STUN auto-retry re-spawns host_thread_fn
 *     (direct_p2p.c:5202);
 *   - the "reuse the live mapping" shortcut is gated on that same
 *     s_upnp_mapping.active (direct_p2p.c:3121), so it is SKIPPED;
 *   - try_portmap runs a second time (direct_p2p.c:3157) and spawns a
