 ...
 *     it is still inside Natpmp_AddMapping (direct_p2p.c:1968), and
 *     try_portmap returns false WITHOUT adopting the result: the
 *     s_upnp_mapping assignment sits on the joined path only
 *     (direct_p2p.c:2058), so s_upnp_mapping.active stays false;
 ...
 *   - try_portmap runs a second time (direct_p2p.c:2718) and spawns a
