 ...
 *   direct_p2p.c:1841-1843 — the race's one shared receive path routes
 ...
 * drain AND its main-thread CHALLENGE echo (direct_p2p.c:4358), and the
...
/* direct_p2p.c:3222 calls STUN_DISCOVER(&s_work.stun, ...) — the mock is
 ...
 * (direct_p2p.c:5259-5267), so a state-only wait can return with the
...
static int test_host_cookie_handshake(void) {
...

    {
        ...
        SDL_snprintf(url, sizeof(url), "udp://127.0.0.1:%u", (unsigned)server_port);
        Config_SetString(CFG_KEY_NETPLAY_DIRECT_P2P_SIGNAL_URL, url);
        /* 1000 ms is the code's own floor (direct_p2p.c:2713); the
     ...
     * direct_p2p.c:1603/1622 and is now frozen in the room code; moving
     ...
     *    flag (direct_p2p.c:3060), Tick observes it, joins the worker,
     *    publishes HANDOFF and calls do_handoff
     *    (direct_p2p.c:5259-5267). A state-only wait is satisfied one
    ...
    if (!tick_until_state(DIRECT_P2P_HANDOFF, 15000)) {
         ...
         * (direct_p2p.c:4813, and :4125 on the direct-receive rung).
 ...
 * when the race's challenge_any evidence was set (direct_p2p.c:3741).
 ...
 *       (src/netplay/direct_p2p.c:2347). So: every DirectP2P_BeginHost
    ...
    while (fgets(buf, sizeof(buf), f) != NULL) {
        ...
        if (strstr(line, key_upnp_off) != NULL) {
            fprintf(stderr,
                    ...
                    "flag (src/netplay/direct_p2p.c:2239); with it clear, the next "
        ...
        if (strstr(line, key_beginhost) != NULL) {
            ...
            if (last_disable_line == 0 ||
                ...
                fprintf(stderr,
                        ...
                        "(src/netplay/direct_p2p.c:2239) — this test would run real "
 ...
 * which side we are (direct_p2p.c:3897). It is a LITERAL at every call
...
static int test_race_deliver_overlaps_seed(void) {
...

    {
         ...
         * (direct_p2p.c:3263 do_handoff). It is passed as a LITERAL at
         ...
         * (direct_p2p.c:4129). The host half is pinned at runtime in
 ...
 * (direct_p2p.c:1512-1517) is BYTE-IDENTICAL to the pre-fix tree:
 *
 ...
 * direct_p2p.c:1524-1537 — and it is TWO changes, not one:
 *
 ...
 * (direct_p2p.c:1939), so no further REGISTER is ever sent — but the
 * REND_FRAME_DELIVER branch of the receive path (direct_p2p.c:1880) is
 ...
 * loop skips any candidate with armed == false (direct_p2p.c:1858) the
...
static bool sb6_run_two_peer(int skew_ms, int owd_ms,
     ...
     * treats a NULL signal_ip as "no legs" (direct_p2p.c:2210-2227). The
