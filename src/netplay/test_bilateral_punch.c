 ...
 *   direct_p2p.c:1835-1837 — the race's one shared receive path routes
 ...
 * drain AND its main-thread CHALLENGE echo (direct_p2p.c:4332), and the
...
/* direct_p2p.c:3206 calls STUN_DISCOVER(&s_work.stun, ...) — the mock is
 ...
 * (direct_p2p.c:5233-5241), so a state-only wait can return with the
...
static int test_host_cookie_handshake(void) {
     ...
     * direct_p2p.c:1597/1501 and is now frozen in the room code; moving
     ...
     *    flag (direct_p2p.c:3044), Tick observes it, joins the worker,
     *    publishes HANDOFF and calls do_handoff
     *    (direct_p2p.c:5233-5241). A state-only wait is satisfied one
    ...
    if (!tick_until_state(DIRECT_P2P_HANDOFF, 15000)) {
         ...
         * (direct_p2p.c:4787, and :3953 on the direct-receive rung).
 ...
 * when the race's challenge_any evidence was set (direct_p2p.c:3721).
 ...
 *       (src/netplay/direct_p2p.c:2331). So: every DirectP2P_BeginHost
 ...
 * which side we are (direct_p2p.c:3871). It is a LITERAL at every call
 ...
 * (direct_p2p.c:1434-1439) is BYTE-IDENTICAL to the pre-fix tree:
 *
 ...
 * direct_p2p.c:1518-1531 — and it is TWO changes, not one:
 *
 ...
 * (direct_p2p.c:1923), so no further REGISTER is ever sent — but the
 * REND_FRAME_DELIVER branch of the receive path (direct_p2p.c:1864) is
...
static bool sb6_run_two_peer(int skew_ms, int owd_ms,
     ...
     * treats a NULL signal_ip as "no legs" (direct_p2p.c:2194-2211). The
