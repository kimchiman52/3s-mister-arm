...
static int test_host_cookie_handshake(void) {
     ...
     * direct_p2p.c:1598/1622 and is now frozen in the room code; moving
    ...
    if (!tick_until_state(DIRECT_P2P_HANDOFF, 15000)) {
         ...
         * (direct_p2p.c:4808, and :4125 on the direct-receive rung).
    ...
    while (fgets(buf, sizeof(buf), f) != NULL) {
        ...
        if (strstr(line, key_upnp_off) != NULL) {
            fprintf(stderr,
                    ...
                    "flag (src/netplay/direct_p2p.c:2234); with it clear, the next "
        ...
        if (strstr(line, key_beginhost) != NULL) {
            ...
            if (last_disable_line == 0 ||
                ...
                fprintf(stderr,
                        ...
                        "(src/netplay/direct_p2p.c:2234) — this test would run real "
...
static int test_race_deliver_overlaps_seed(void) {
...

    {
         ...
         * (direct_p2p.c:3258 do_handoff). It is passed as a LITERAL at
         ...
         * (direct_p2p.c:4124). The host half is pinned at runtime in
 ...
 * (direct_p2p.c:1507-1512) is BYTE-IDENTICAL to the pre-fix tree:
 *
 ...
 * loop skips any candidate with armed == false (direct_p2p.c:1853) the
