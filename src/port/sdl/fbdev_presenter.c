...
static bool compute_fps_overlay_layout(const SDL_FRect* content_rect, FpsOverlayLayout* out_layout) {
    ...
    /* perf-3: support a multi-line overlay by splitting on '\n'. We compute
     * the widest line for the bounding box and total height = lines * glyph_h
     * + (lines - 1) * line_gap. The rasterizer below renders each line at
     * the corresponding y offset. */
    int line_count = 1;
    ...
    if (fps_overlay_mode == 1) {
         ...
         * the same y as the legacy single-line overlay (i.e., extra lines
         * extend upward). */
        draw_x = x0 + ((content_w - text_w) / 2);
        draw_y = y1 - total_h - safe_margin;
    }
    ...
    const int bg_y1 = clamp_to_range(draw_y + total_h + bg_pad, 0, fb_height);
...
static bool ensure_rasterized_fps_overlay(const FpsOverlayLayout* layout, bool* out_changed) {
    ...
    const int text_len = (int)SDL_strlen(fps_overlay_text);
    /* perf-3: walk the overlay text glyph-by-glyph; on '\n' advance the y
     * offset by glyph_h + line_gap and reset the column index. */
    int line_index = 0;
    int col_index = 0;
    for (int i = 0; i < text_len; i++) {
...
/* perf-2 565 sibling. Per-pixel ARGB->565 pack at FPS-overlay rate is
 * trivial CPU (overlay is at most ~32x7 px); avoids carrying a separate
 * 565 atlas cache field. */
static void apply_rasterized_fps_overlay_to_rgb565_buffer(Uint16* pixels, int width, int height, int stride_pixels) {
    if (!fps_overlay_frame_active || (pixels == NULL) || (fps_overlay_pixels == NULL) || (stride_pixels < width)) {
        return;
    }
...
/* perf-3: count lines (separated by '\n') in fps_overlay_text and the widest
 * line's char count. Both ApplyFPSOverlayTo* layout helpers below use this
 * to size the bg rect for a multi-line debug overlay; without it line 2
 * renders into an undersized buffer and is clipped offscreen. */
