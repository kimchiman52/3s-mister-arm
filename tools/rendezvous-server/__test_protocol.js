...
async function testKeyBudgetCoversLegitimateSignalling(handle) {
    ...
    // endpoint, src/netplay/direct_p2p.c:2883-2885), so a room's real
...
async function testKeyBudgetCoversMultiJoinerRoom(handle) {
    ...
    // HOST's public endpoint (src/netplay/direct_p2p.c:2883-2885), so the
    // key IS the room code. Everyone who pastes that code hashes to the
    // same key, and each one starts its 500 ms REGISTER resend
    // (direct_p2p.c:1430-1431 -> 2/s) the moment the code is entered,
    // without waiting to be accepted (cfg.signal_leg, direct_p2p.c:2925,
    ...
    // 1000 ms, direct_p2p.c:2346-2348 -> 1/s). Its packets are what
