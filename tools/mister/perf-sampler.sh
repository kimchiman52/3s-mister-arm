...
while [ "$#" -gt 0 ]; do
    ...
    --perf-basic-first-window-render-subphases)
        ...
        ;;
    --perf-basic-first-window-exact-hot-family-alpha-offpath)
        perf_basic_first_window_exact_hot_family_alpha_offpath=1
        shift
        ;;
    --perf-basic-first-window-onset-exact-hot-family-alpha-offpath)
...
if [ "$perf_basic_first_window_exact_hot_family_alpha_offpath" -eq 1 ] &&
    [ "$perf_basic_first_window_families" -eq 0 ]; then
    echo "error: --perf-basic-first-window-exact-hot-family-alpha-offpath requires --perf-basic-first-window-families." >&2
    exit 2
fi

if [ "$perf_basic_first_window_onset_exact_hot_family_alpha_offpath" -eq 1 ] &&
    [ "$perf_basic_first_window_families" -eq 0 ]; then
    echo "error: --perf-basic-first-window-onset-exact-hot-family-alpha-offpath requires --perf-basic-first-window-families." >&2
    exit 2
fi
...
if [ "$perf_basic_first_window_exact_hot_family_alpha_offpath" -eq 1 ]; then
    extra_app_args="${extra_app_args} --perf-basic-first-window-exact-hot-family-alpha-offpath"
fi
